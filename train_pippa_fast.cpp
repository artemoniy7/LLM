...
int main() {
    ...
    try {
        ...
        // Загружаем только 20 диалогов
        std::cout << "\nLoading PIPPA dataset (first 20 dialogues only)...\n";
        std::string filename = "data/pippa_sample.jsonl";
        
        auto dialogues = load_pippa_dataset(filename, 30);
        std::cout << "Loaded " << dialogues.size() << " dialogues\n";
        
        if (dialogues.empty()) {
            std::cerr << "Error: No dialogues loaded!\n";
            std::cerr << "Make sure the file exists at: " << filename << "\n";
            return 1;
        }
